...
def test_r2_config_prefers_env_over_file(monkeypatch) -> None:
    ...
    monkeypatch.setenv("R2_ENDPOINT_URL", "https://env-override.r2.example")
    ...
    assert r2.endpoint_url == "https://env-override.r2.example"
...
def test_r2_config_allows_endpoint_without_account_id(monkeypatch) -> None:
    monkeypatch.delenv("R2_ACCOUNT_ID", raising=False)
    monkeypatch.setenv("R2_ENDPOINT_URL", "https://example.eu.r2.cloudflarestorage.com")
    monkeypatch.setenv("R2_BUCKET", "b")
    cfg = AppConfig(r2=R2FileConfig())
    r2 = R2Config.from_env_or_config(cfg)
    assert r2.endpoint_url == "https://example.eu.r2.cloudflarestorage.com"
